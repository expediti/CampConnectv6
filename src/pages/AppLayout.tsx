...
export default function AppLayout() {
  ...
  useEffect(() => {
    ...
    console.log('🔴 Real-time listening for community:', currentCommunity.id);
  ...
  return (
    <>
      ...
      {/* FLOATING BUTTON - OUTSIDE MAIN CONTAINER - ALWAYS VISIBLE */}
      {view === 'chat' && currentCommunity && (
        <button 
          onClick={() => setShowMessageModal(true)} 
          className="fixed bottom-8 right-8 bg-green-600 hover:bg-green-700 active:scale-95 text-white w-16 h-16 rounded-full shadow-2xl hover:shadow-green-500/50 transition-all flex items-center justify-center text-3xl cursor-pointer"
          style={{
            position: 'fixed',
            zIndex: 9999,
            bottom: '32px',
